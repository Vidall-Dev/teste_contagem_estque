  ...
  const stocks = [
    {
      ...
      description: 'Contagem geral de todo o armazém central. Recomendado para fechamentos mensais.',
      icon: Warehouse,
      color: 'text-primary',
    ...
    },
    {
      ...
      description: 'Contagem de produtos em trânsito ou em docas de expedição imediata.',
      icon: Truck,
      color: 'text-blue-600',
    ...
    {
      id: 'area-devolucao',
      title: 'Área de Devolução',
      description: 'Itens retornados que precisam de conferência de integridade e re-estocagem.',
      icon: RotateCcw,
      ...
      <div className="max-w-4xl mx-auto flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
          {stocks.map((stock) => (
            <Card
              key={stock.id}
              hoverable
              active={stock.id === 'deposito-principal'}
              className="p-8 flex flex-col h-full group"
            >
              <div className={`w-14 h-14 rounded-2xl ${stock.bgColor} flex items-center justify-center mb-6 ${stock.color}`}>
                <stock.icon className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold mb-3 text-gray-900 dark:text-white">{stock.title}</h3>
        ...
        <div className="w-full max-w-lg mb-8">
          <Button
            ...
            onClick={() => navigate('/new-count/step-2')}
          >
